...
def add_secrets_manager_policy(role_name: str):
    """
    Add Secrets Manager access policy to the execution role.
    
    Args:
        role_name (str): Name of the IAM role to modify
    """
    ...
    try:
        ...
        print(f"Added Secrets Manager policy to role: {role_name}")
    except Exception as e:
        print(f"Failed to add Secrets Manager policy: {e}")
...
def deploy_agentcore(agent_name: str, entry_point: str, requirements_file: str = 'requirements.txt', local_build: bool = False):
    ...
    # Add Secrets Manager access to the auto-created role
    try:
        status_response = agentcore_runtime.status()
        ...
        if role_name:
            add_secrets_manager_policy(role_name)
        else:
            print("Could not determine role name from runtime status")
    except Exception as e:
        print(f"Could not add Secrets Manager policy: {e}")
