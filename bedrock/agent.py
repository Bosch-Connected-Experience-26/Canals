...
def place_lookup_by_country(query_str: str) -> str:
    ...
    try:
        ...
        collection = get_travel_collection(client, country=query_str)
        res = ""
        ...
        logger.error(f"Error looking up places by country '{query_str}': {e}")
        raise

EUROPEAN_COUNTRIES = {
    "germany", "france", "italy", "spain", "portugal", "netherlands",
    "belgium", "switzerland", "austria", "sweden", "norway", "denmark",
    "finland", "poland", "czechia", "czech republic", "hungary", "greece",
    "croatia", "romania", "europe",
}

def get_travel_collection(client, country: str = ""):
    logger.info("Getting travel collection from MongoDB")
    db = client['travel']
    collection_name = "europe" if country.lower().strip() in EUROPEAN_COUNTRIES else "asia"
    logger.info(f"Routing country '{country}' to collection '{collection_name}'")
    return db[collection_name]

@tool
def place_lookup_by_name(query_str: str, country: str = "") -> str:
    """Retrieve place information by place name

    Args:
        query_str: The place name to search for
        country: The country the place is in, used to select the correct dataset
    ...
    try:
        client = get_mongo_client()
        collection = get_travel_collection(client, country=country)
        res = ""
        filter = {
            "$or": [
        ...
        logger.error(f"Error looking up place by name '{query_str}': {e}")
        raise

@tool
def place_best_time_lookup(query_str: str, country: str = "") -> str:
    ...
    try:
        ...
        logger.error(f"Error looking up best time to visit for '{query_str}': {e}")
        raise
...
def setup_bedrock():
    ...
    # amazon.titan-embed-text-v1 is not available in eu-north-1; eu-central-1 has it
    return boto3.client(
...
@tool
def mongodb_search(query: str, country: str = "") -> str:
    """Retrieve place information by place features using semantic search

    Args:
        query: Description of place features to search for
        country: The country or region to search in, used to select the correct dataset
